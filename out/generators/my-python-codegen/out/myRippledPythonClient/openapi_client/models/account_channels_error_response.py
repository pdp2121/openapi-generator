...
class AccountChannelsErrorResponse(BaseModel):
    """
    AccountChannelsErrorResponse
    """ # noqa: E501
    error: AccountChannelsErrorResponseError
    status: StrictStr
    request: AccountChannelsRequest
    __properties: ClassVar[List[str]] = ["error", "status", "request"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.error:
            _dict_snake_case['error'] = self.error.to_dict()
        # override the default output from pydantic by calling `to_dict()` of request
        if self.request:
            _dict_snake_case['request'] = self.request.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "error": AccountChannelsErrorResponseError.from_dict(obj["error"]) if obj.get("error") is not None else None,
            "status": obj.get("status"),
            "request": AccountChannelsRequest.from_dict(obj["request"]) if obj.get("request") is not None else None
        })
        return _obj
